...
<div class="content">
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">แก้ไขคลินิก</h3>
        ...
        <div class="block-content">
            ...
            <form action="{{ route('clinics.update', $clinic) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('group_id') is-invalid @enderror" id="group_id" name="group_id" required>
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('group_id', $clinic->group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4">{{ old('description', $clinic->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
